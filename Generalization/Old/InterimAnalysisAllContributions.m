...
for i=1:length(groups)
    ...
    for cc=1:parameter  %remove baseline manually!
        ...
        ot.OGLA{cc}=cell2mat(getGroupedData(dt,{'spatialContributionNorm2','stepTimeContributionNorm2','velocityContributionNorm2','netcontributionNorm2'},{'OG post'},0,-1*20,10,0));
...
%         else
            ...
            for j=1:parameter %creating cells for each parameter. Each cell contains 10 subjects
               ...
               % keyboard
                eval(['AllData.',groups{i},'.OGpMax{j}(sj,1)=findmaxAV(ot.OGp{j}(:,sj),2,20,5)'])
                eval(['AllData.',groups{i},'.TMpMax{j}(sj,1)=findmaxAV(ot.TMp{j}(:,sj),2,20,5)'])
...
for i=1:length(groups)
    ...
    eval(['dt6=AllData.',groups{i},'.OGpMax{4};';])
    eval(['dt7=AllData.',groups{i},'.TMpMax{4};';])
    Context1{i}=(dt2./dt1)*100;
    ContextTM{i}=(dt4./dt5)*100;
    OGtoTM1(:,i)=(dt3./dt4)*100;    
    OGtoTM2(:,i)=(dt6./dt7)*100;
end
  clear dt1 dt2 dt3 dt4 dt5
 

%% These are the first figures we made in september, I created a new script regarding figures so this is commened for now. 
% %%%%%%%%%%%%%%
% %make figures%
% %%%%%%%%%%%%%%
% 
