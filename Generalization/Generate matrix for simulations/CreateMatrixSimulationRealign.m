...
for c=1:length(commonConditions)
    ...
    for g=1:length(names)
        %create ss matrix
        allData{g}{c}.net= squeeze(cell2mat(studyData.(names{g}).getGroupedData('netContributionNorm2',commonConditions{c},0,maxn(c),0,10,1)));
        allData{g}{c}.vel= squeeze(cell2mat(studyData.(names{g}).getGroupedData('velocityContributionNorm2',commonConditions{c},0,maxn(c),0,10,1)));
...
for g=1:length(names)
    ...
    if g==4;%his is the catch group
        ...
        for sj=1:10
            ...
            for tr=Adapt
                if tr<Catch
                    netBeforeCatch{sj}=[netBeforeCatch{sj};studyData.Catch.adaptData{sj}.getParamInTrial('netContributionNorm2',tr)];
                    velBeforeCatch{sj}=[velBeforeCatch{sj};studyData.Catch.adaptData{sj}.getParamInTrial('velocityContributionNorm2',tr)];
                    
                    
                else
                    netAfterCatch{sj}=[netAfterCatch{sj};studyData.Catch.adaptData{sj}.getParamInTrial('netContributionNorm2',tr)];
                    velAfterCatch{sj}=[velAfterCatch{sj};studyData.Catch.adaptData{sj}.getParamInTrial('velocityContributionNorm2',tr)];
        ...
        %get catch data
        CatchDataNet=[squeeze(cell2mat(studyData.(names{g}).getGroupedData('netContributionNorm2','Catch',0,10,0,0,1)))];
        CatchDataVel=[squeeze(cell2mat(studyData.(names{g}).getGroupedData('velocityContributionNorm2','Catch',0,10,0,0,1)))];
        %Running Avg
%         for sj=1:10
...
%         end
        ...
        allData{g}{8}.net= squeeze(cell2mat(studyData.(names{g}).getGroupedData('netContributionNorm2','gradual adaptation',0,1200,0,10,1)));
        allData{g}{8}.vel= squeeze(cell2mat(studyData.(names{g}).getGroupedData('velocityContributionNorm2','gradual adaptation',0,1200,0,10,1)));
        if g==2
        else
