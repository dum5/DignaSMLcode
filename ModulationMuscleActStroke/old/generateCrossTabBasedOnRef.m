function [dataBinary,dataCount,ATS]=generateCrossTab(this,epoch,refepoch,Prefix,mindif,signmatrix,fdr,refgroup1,refgroup2)
%keyboard

%first determine which epochs are significant in controls and patients
%this code is not very efficient, but works for now

ft=figure;
ac=subplot(1,1,1);
[ft,ac,labelsD,dataEcD,dataRefcD]=plotCheckerboards(refgroup1,Prefix,epoch,ft,ac,refepoch,false,'nanmedian');%make sure that flipLR is off, because biniary data are also not time shifted.
close gcf;
...
ft=figure;
ac=subplot(1,1,1);
[ft,ac,labelsD,dataEcD,dataRefcD]=plotCheckerboards(refgroup2,Prefix,epoch,ft,ac,refepoch,false,'nanmedian');%make sure that flipLR is off, because biniary data are also not time shifted.
close gcf;
[pvalsD,hsD,alphaAdj_cD]=checkerstats(dataEcD,[],1,0,fdr,'benhoch',mindif);
...
dt=nanmedian(epData{1},2)-nanmedian(refData{1},2); 
 ...
 for sj=1:size(refData{1},4)
    ...
    %  keyboard
      [ht2,alphaAdj,i1] = BenjaminiHochberg(pt(ind),fdr);
      h=NaN(360,1);h(ind,1)=ht2;
      h=h.*signrow;
      %h=ht.*signrow;
      allh(:,sj)=h;
