...
stancecontroldata.baselineData.fastKnee=NaN(10,150);
stancecontroldata.baselineData.slowKnee=NaN(10,150);
stancecontroldata.AdaptData.fastKnee=NaN(10,950);
stancecontroldata.AdaptData.slowKnee=NaN(10,950);

stancestrokedata.baselineData.fastKnee=NaN(10,150);
stancestrokedata.baselineData.slowKnee=NaN(10,150);
stancestrokedata.AdaptData.fastKnee=NaN(10,950);
stancestrokedata.AdaptData.slowKnee=NaN(10,950);

for c=1:10
    meancontroldata.baselineData.fastLimb(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastLimb);
    meancontroldata.baselineData.slowLimb(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowLimb);
    meancontroldata.baselineData.fastHip(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastHip);
    meancontroldata.baselineData.slowHip(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowHip);
    meancontroldata.baselineData.fastKnee(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastKnee);
    meancontroldata.baselineData.slowKnee(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowKnee);
    meancontroldata.baselineData.fastAnk(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastAnk);
    meancontroldata.baselineData.slowAnk(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowAnk);
    
    meancontroldata.AdaptData.fastLimb(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastLimb(end-39:end,:));
    meancontroldata.AdaptData.slowLimb(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowLimb(end-39:end,:));
    meancontroldata.AdaptData.fastHip(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastHip(end-39:end,:));
    meancontroldata.AdaptData.slowHip(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowHip(end-39:end,:));
    meancontroldata.AdaptData.fastKnee(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastKnee(end-39:end,:));
    meancontroldata.AdaptData.slowKnee(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowKnee(end-39:end,:));
    meancontroldata.AdaptData.fastAnk(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastAnk(end-39:end,:));
    meancontroldata.AdaptData.slowAnk(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowAnk(end-39:end,:));
    
    
    meanstrokedata.baselineData.fastLimb(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastLimb);
    meanstrokedata.baselineData.slowLimb(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowLimb);
    meanstrokedata.baselineData.fastHip(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastHip);
    meanstrokedata.baselineData.slowHip(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowHip);
    meanstrokedata.baselineData.fastKnee(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastKnee);
    meanstrokedata.baselineData.slowKnee(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowKnee);
    meanstrokedata.baselineData.fastAnk(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastAnk);
    meanstrokedata.baselineData.slowAnk(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowAnk);
    
    meanstrokedata.AdaptData.fastLimb(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastLimb(end-39:end,:));
    meanstrokedata.AdaptData.slowLimb(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowLimb(end-39:end,:));
    meanstrokedata.AdaptData.fastHip(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastHip(end-39:end,:));
    meanstrokedata.AdaptData.slowHip(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowHip(end-39:end,:));
    meanstrokedata.AdaptData.fastKnee(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastKnee(end-39:end,:));
    meanstrokedata.AdaptData.slowKnee(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowKnee(end-39:end,:));
    meanstrokedata.AdaptData.fastAnk(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastAnk(end-39:end,:));
    meanstrokedata.AdaptData.slowAnk(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowAnk(end-39:end,:));
    
    
    meancontroldata.baselineData.fastHipVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastHipVEL);
    meancontroldata.baselineData.slowHipVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowHipVEL);
    meancontroldata.baselineData.fastKneeVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastKneeVEL);
    meancontroldata.baselineData.slowKneeVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowKneeVEL);
    meancontroldata.baselineData.fastAnkVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.fastAnkVEL);
    meancontroldata.baselineData.slowAnkVEL(c,:)=nanmean(controldata(cnrs(c)).baselineData.slowAnkVEL);
    
    meancontroldata.AdaptData.fastHipVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastHipVEL(end-39:end,:));
    meancontroldata.AdaptData.slowHipVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowHipVEL(end-39:end,:));
    meancontroldata.AdaptData.fastKneeVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastKneeVEL(end-39:end,:));
    meancontroldata.AdaptData.slowKneeVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowKneeVEL(end-39:end,:));
    meancontroldata.AdaptData.fastAnkVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.fastAnkVEL(end-39:end,:));
    meancontroldata.AdaptData.slowAnkVEL(c,:)=nanmean(controldata(cnrs(c)).AdaptData.slowAnkVEL(end-39:end,:));
    
    
    meanstrokedata.baselineData.fastHipVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastHipVEL);
    meanstrokedata.baselineData.slowHipVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowHipVEL);
    meanstrokedata.baselineData.fastKneeVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastKneeVEL);
    meanstrokedata.baselineData.slowKneeVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowKneeVEL);
    meanstrokedata.baselineData.fastAnkVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.fastAnkVEL);
    meanstrokedata.baselineData.slowAnkVEL(c,:)=nanmean(strokedata(pnrs(c)).baselineData.slowAnkVEL);
    
    meanstrokedata.AdaptData.fastHipVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastHipVEL(end-39:end,:));
    meanstrokedata.AdaptData.slowHipVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowHipVEL(end-39:end,:));
    meanstrokedata.AdaptData.fastKneeVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastKneeVEL(end-39:end,:));
    meanstrokedata.AdaptData.slowKneeVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowKneeVEL(end-39:end,:));
    meanstrokedata.AdaptData.fastAnkVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.fastAnkVEL(end-39:end,:));
    meanstrokedata.AdaptData.slowAnkVEL(c,:)=nanmean(strokedata(pnrs(c)).AdaptData.slowAnkVEL(end-39:end,:));
    
    stancecontroldata.baselineData.fastKnee(c,1:145)=nanmean(controldata(cnrs(c)).baselineData.fastKnee(end-144:end,1:45)');
    stancecontroldata.baselineData.slowKnee(c,1:145)=nanmean(controldata(cnrs(c)).baselineData.slowKnee(end-144:end,1:45)');
    stancecontroldata.AdaptData.fastKnee(c,1:length(controldata(cnrs(c)).AdaptData.fastKnee))=nanmean(controldata(cnrs(c)).AdaptData.fastKnee(:,1:45)');
    stancecontroldata.AdaptData.slowKnee(c,1:length(controldata(cnrs(c)).AdaptData.slowKnee))=nanmean(controldata(cnrs(c)).AdaptData.slowKnee(:,1:45)');
    
    stancestrokedata.baselineData.fastKnee(c,1:145)=nanmean(strokedata(pnrs(c)).baselineData.fastKnee(end-144:end,1:45)')';
    stancestrokedata.baselineData.slowKnee(c,1:145)=nanmean(strokedata(pnrs(c)).baselineData.slowKnee(end-144:end,1:45)')';
    stancestrokedata.AdaptData.fastKnee(c,1:length(strokedata(pnrs(c)).AdaptData.fastKnee))=nanmean(strokedata(pnrs(c)).AdaptData.fastKnee(:,1:45)')';
    stancestrokedata.AdaptData.slowKnee(c,1:length(strokedata(pnrs(c)).AdaptData.slowKnee))=nanmean(strokedata(pnrs(c)).AdaptData.slowKnee(:,1:45)')';
...
title('Knee Angle Velocity during Swing')
...
title('Knee angle velocity during Swing Adap-Bas')
