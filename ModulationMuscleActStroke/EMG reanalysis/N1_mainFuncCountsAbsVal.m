...
ha=tight_subplot(2,3,[.03 .005],.04,.04);
pc1=ha(1,1);
ps1=ha(2,1);
pc2=ha(1,2);
ps2=ha(2,2);
...
hb=tight_subplot(2,3,[.03 .005],.04,.04);
pd1=hb(1,1);
pd2=hb(1,2);
pd3=hb(1,3);
...
[fi,fb,p1c,ps1,pd1,pvalc1,pvals1,pvalb1,hc1,hs1,hb1,dataEc1,dataEs1]=plotBGcompCountsAbsVal(fi,fb,pc1,ps1,pd1,eps,reps,newLabelPrefix,groups,0.05,0.1);

[eps] = defineEpochs({'eA'},{'Adaptation'}',[15],[eE],[eL],'nanmedian');
[reps] = defineEpochs({'Base'},{'TM base'}',[-40],[eE],[eL],'nanmedian');
[fi,fb,p2c,ps2,pd2,pvalc2,pvals2,pvalb2,hc2,hs2,hb2,dataEc2,dataEs2]=plotBGcompCountsAbsVal(fi,fb,pc2,ps2,pd2,eps,reps,newLabelPrefix,groups,0.05,0.1);

[eps] = defineEpochs({'eP'},{'Washout'}',[15],[eE],[eL],'nanmedian');
[reps] = defineEpochs({'lA'},{'Adaptation'}',[-40],[eE],[eL],'nanmedian');
[fi,fb,p3c,ps3,pd3,pvalc3,pvals3,pvalb3,hc3,hs3,hb3,dataEc3,dataEs3]=plotBGcompCountsAbsVal(fi,fb,pc3,ps3,pd3,eps,reps,newLabelPrefix,groups,0.05,0.1);


whiteMiddleColorMap;
set(fi,'ColorMap',map2);
set(ha(1:2,1),'CLim',[-1 1].*15);
set(ha(1:2,2:end),'YTickLabels',{},'CLim',[-1 1].*15);
set(ha(1,:),'XTickLabels','');
set(ha(2,:),'XTick',(0:numel(evLabel)-1)/12,'CLim',[-1 1]*15);
set(ha,'FontSize',8)
