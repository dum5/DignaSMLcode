...
function [fi,fb,pc,ps,pd,pvalc,pvals,pvalb,hc,hs,hb,dataBinaryc,dataBinarys]=plotBGcompCountsAbsVal(fi,fb,pc,ps,pd,epoch,refepoch,Prefix,groups,mindif,fdr)
...
[dataBinaryc,dataCountc,ATSc]=generateCrossTabBasedOnRef(groups{1},epoch,refepoch,Prefix,mindif,signMatrix,fdr,groups{1},groups{2});
%ATSc.Data=abs(ATSc.Data);
ATSc.plotCheckerboard(fi,pc); colorbar off
...
[dataBinarys,dataCounts,ATSs]=generateCrossTabBasedOnRef(groups{2},epoch,refepoch,Prefix,mindif,signMatrix,fdr,groups{1},groups{2});
%ATSs.Data=abs(ATSs.Data);
ATSs.plotCheckerboard(fi,ps); colorbar off
...
ft=figure
ac=subplot(1,1,1)
[ft,ac,labelsD,dataEcD,dataRefcD]=plotCheckerboards(groups{1},Prefix,epoch,ft,ac,refepoch,true,'nanmedian');
close gcf;

[pvalcD,hcD,alphaAdj_cD]=checkerstats(dataEcD,[],1,0,fdr,'benhoch',mindif);

ATSd=alignedTimeSeries(0,1/numel(evLabel),[abs(dataCountc(:,:,1))-abs(dataCounts(:,:,1))],Prefix,ones(1,Np),evLabel);
ATSd.plotCheckerboard(fb,pd);colorbar off
...
for i=1:size(hc,1)
    for k=1:size(hc,2)        
       %if hb(i,k)==1 %&& (hc(i,k)==1 || hs(i,k)==1)
        if hb(i,k)==1 %&& hc(i,k)==1
            plot3((k-0.5)/12,i-0.5,20,'.','MarkerSize',16,'Color','k')
        end
       if hcD(i,k)==1
           plot3((k-0.65)/12,i-0.5,20,'.','MarkerSize',10,'Color','k','Marker','*')
       end
