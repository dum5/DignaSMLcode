...
f1=figure;
set(f1,'Color',[1 1 1]','Units','inches','Position',[0 0 7 10]);
ph=tight_subplot(5,3,0.05,0.1,0.08);
set(ph,'XTickLabelMode','auto','YTickLabelMode','auto','FontSize',12,'Box','off')
set(ph(1:4,1),'XLim',[20 35],'XTick',[20 25 30 35])%FM
set(ph(1:4,2),'XLim',[40 80],'XTick',[40 60 80])%Age
set(ph(1:4,3),'XLim',[0 1.5],'XTick',[0 0.5 1 1.5])%velocity
...
set(ph(5,1),'XLim',[0 10],'XTick',[0 5 10]);
set(ph(5,2),'XLim',[-0.2 1.2],'XTick',[0 0.5 1])
set(ph(5,3),'XLim',[-10 20],'XTick',[-10 0 10 20])


ph(1:4,:)=moveAxis(ph(1:4,:),0.04,0);
ph(5,:)=moveAxis(ph(5,:),-0.02,0);
ph=moveAxis(ph,0,0.02);

xlabel(ph(4,1),'Fugl-Meyer')
xlabel(ph(4,2),'Age')
xlabel(ph(4,3),'Velocity')
...
set(ph(1:3,:),'XTickLabel',{''})

xlabel(ph(5,1),'|| LateA ||');ylabel(ph(5,1),'\beta_M')
xlabel(ph(5,2),'EMG_Q_u_a_d LateA')
xlabel(ph(5,3),'\theta Knee_s_l_o_w LateA')
...
text(ph(5,1),-3,1.2,'B','FontSize',14,'FontWeight','bold')
%correlations between feeback and feedforward adaptation
[a]=plotCor(ph(5,1),TStroke.lAMagn(Idx),TStroke.BM(Idx),TControls.lAMagn(Idx),TControls.BM(Idx));
[a]=plotCor(ph(5,2),TStroke.FF_Quad(Idx),TStroke.BM(Idx),TControls.FF_Quad(Idx),TControls.BM(Idx));
[a]=plotCor(ph(5,3),TStroke.FF_skneeAngleAtSHS(Idx),TStroke.BM(Idx),TControls.FF_skneeAngleAtSHS(Idx),TControls.BM(Idx));
...
if nargin>3
    ...
    tc=text(ax,xl(1),yl(2)+diff(yl)/8,['rho= ',num2str(round(rhoc(2),2)),' p=',num2str(round(pc(2),2))]);set(tc,'Color',[0.7 0.7 0.7],'FontSize',12,'FontWeight','bold')
    if pc(2)<0.05
...
ts=text(ax,xl(1),yl(2),['rho= ',num2str(round(rhos(2),2)),' p=',num2str(round(ps(2),2))]);set(ts,'Color',[0 0 0],'FontSize',12,'FontWeight','bold')
if ps(2)<0.05
