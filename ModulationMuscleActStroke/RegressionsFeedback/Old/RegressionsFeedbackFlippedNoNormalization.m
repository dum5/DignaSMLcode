...
if allSubFlag==0; %if this is set to 1, the bad subjects are in the analysis, so don't run the group analysis
    %note that tables are flipped
    if groupMedianFlag
        ttS=table(-median(eA_C(:,cIdx),2), median(eAT_C(:,cIdx),2), -median(lA_C(:,cIdx),2), median(eP_S(:,cIdx),2)-median(lA_S(:,cIdx),2),'VariableNames',{'eA','eAT','lA','eP_lA'});
        %patients that are don't have good data will be removed
        ttC=table(-median(eA_S(:,pIdx),2), median(eAT_S(:,pIdx),2), -median(lA_S(:,pIdx),2), median(eP_C(:,pIdx),2)-median(lA_C(:,pIdx),2),'VariableNames',{'eA','eAT','lA','eP_lA'});
    else
...
%     figure
%     hold on
%     bar([1 3.5],[CmodelFit2.Coefficients.Estimate(1) CmodelFit1.Coefficients.Estimate(1)],'FaceColor',[1 1 1],'EdgeColor',[0 0 0],'LineWidth',2,'BarWidth',0.3)
%     bar([2 4.5],[SmodelFit2.Coefficients.Estimate(1) SmodelFit1.Coefficients.Estimate(1)],'FaceColor',[0 0 0],'EdgeColor',[0 0 0],'LineWidth',2,'BarWidth',0.3)
%     dt=CmodelFit2.coefCI;plot([1 1],dt(1,:),'-k','LineWidth',2,'Color',[0.5 0.5 0.5]);
%     dt=CmodelFit1.coefCI;plot([3.5 3.5],dt(1,:),'-k','LineWidth',2,'Color',[0.5 0.5 0.5]);
%     dt=SmodelFit2.coefCI;plot([2 2],dt(1,:),'-k','LineWidth',2,'Color',[0.5 0.5 0.5]);
...
%     title('N=10 per group')
    ...
    if speedMatchFlag==0
        save([matDataDir,'GroupMedianRegressionFullNoNormFlipped.mat'],'CmodelFit2','SmodelFit2');
    elseif speedMatchFlag==1
        save([matDataDir,'GroupMedianRegressionSpeedMatchNoNormFlipped.mat'],'CmodelFit2','SmodelFit2');
    end
